...
class Publish {
  ...
  publishTo := {
    ...
    sonatypeProfileName := "aparo",
    pomIncludeRepository := { _ =>
      false
    },
    organizationHomepage := Some(url("https://paroconsulting.com")),
    homepage := Some(url("http://paroconsulting.com")),
    licenses := Seq("Apache 2.0" -> url("http://www.apache.org/licenses/LICENSE-2.0")),
    scmInfo := Some(
      ScmInfo(
        url("https://github.com/aparo/" + name.value),
        "scm:git:git@github.com/aparo/" + name.value + ".git"
      )
    ),
    ...
    developers := List(
      Developer(
        ...
        url = new URL("https://paroconsulting.com")
      )
    ),
    // sbt-release
